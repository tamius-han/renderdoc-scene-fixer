...
export class SceneViewerApp {
  ...
  private selectGroundPlaneBtn = this.el<HTMLButtonElement>("select-ground-plane-btn");

  private groundPlaneCancelBtn = this.el<HTMLButtonElement>("select-ground-plane_cancel-selection");
  private groundPlaneAcceptBtn = this.el<HTMLButtonElement>("select-ground-plane_accept-selection");
  private groundPlaneAccept180Btn = this.el<HTMLElement>("select-ground-plane_accept-selection-180");
  ...
  /** Arms/disarms the "Select ground plane" tool - see
   * startGroundPlaneTool()/cancelGroundPlaneTool(). */
  private toggleGroundPlaneTool(): void {
    if (this.groundPlaneToolActive || this.groundPlaneRotationSnapshot) this.cancelGroundPlaneTool();
    else this.startGroundPlaneTool();
  }

  /** Arms the ground-plane tool: unhides fix-distortion-submenu (showing
   * its "no selection yet" placeholder, with the accept/cancel row hidden
   * - see showFixDistortionNoSelection()), snapshots the rotation state so
   * Cancel can restore it later, and switches the viewport cursor to a
   * crosshair to start collecting the next 3 left-clicks on mesh surface
  ...
  private startGroundPlaneTool(): void {
    ...
    this.selectGroundPlaneBtn.classList.add("active");
  }

  /** Aborts the ground-plane tool entirely and restores whatever rotation
   * was active before it was armed (see groundPlaneRotationSnapshot's doc
   * comment) - used for the Cancel button (whether clicked mid-placement
   * or during the post-3rd-point accept/cancel review), a right-click
   * abort mid-placement, and reconstructScene()'s per-reconstruct reset.
   * Harmless to call when the tool isn't active/pending (e.g. from the
   * select-area tool's mutual-exclusion guard) - there's simply nothing to
   * restore or hide in that case. */
  private cancelGroundPlaneTool(): void {
    ...
    this.selectGroundPlaneBtn.classList.remove("active");
    this.hideFixDistortionSubmenu();
  }
   ...
   * turn to match, rather than just negating Y on its own. */
  ...
  /** Shows fix-distortion-submenu's "no selection yet" placeholder and
   * hides its accept/cancel row - the state the submenu starts in each
   * time the tool is (re)armed, before 3 points have been placed. */
  private showFixDistortionNoSelection(): void {
    this.groundPlaneCancelBtn.style.display = "none";
    this.groundPlaneAcceptBtn.style.display = "none";
    this.groundPlaneAccept180Btn.style.display = "none";
  }

  /** Swaps fix-distortion-submenu from "no selection yet" over to the
   * accept/cancel row - called once the 3rd point is placed and the
   * preview rotation has already been computed and applied (see
   * finishGroundPlaneTool()). */
  private showFixDistortionAcceptCancel(): void {
    this.groundPlaneCancelBtn.style.display = "";
    this.groundPlaneAcceptBtn.style.display = "";
    this.groundPlaneAccept180Btn.style.display = "";
  }

  /** Hides fix-distortion-submenu entirely and resets its internal state
   * back to "no selection yet", ready for the next time the tool is armed
   * - called once the tool run is over, whether via Cancel or Accept. */
  private hideFixDistortionSubmenu(): void {
    this.showFixDistortionNoSelection();
  }
   ...
   * accept/cancel row - see showFixDistortionAcceptCancel(). The rotation
   * isn't final yet: it only sticks if the user clicks Accept (or "Accept,
  ...
  private finishGroundPlaneTool(): void {
    ...
    this.groundPlaneToolActive = false;
    this.sceneManager.renderer.domElement.style.cursor = "";
    this.showFixDistortionAcceptCancel();
  }
